...
if(isset($_SESSION['email'])) {
    if ($_SERVER["REQUEST_METHOD"] === "POST") {
        ...
        $query1 = $sql->prepare("UPDATE `id22194774_sepais`.`sepae_libera_aluno` SET `horario_saida` = ?
        WHERE (`aluno_id` = ?) and `data` LIKE ?;");
        $query2 = $sql->prepare("UPDATE `id22194774_sepais`.`responsavel_libera_aluno` SET `horario_saida` = ?
        WHERE (`aluno_id` = ?) and `data` LIKE ? ;");
        // }
        // elseif(isset($_POST['responsavel'])){
        //     $query = $sql->prepare("UPDATE `id22194774_sepais`.`responsavel_libera_aluno` SET `horario_saida` = $horario_saida
        //     WHERE (`aluno_id` = ?) and `data` LIKE ?;");
        // }
