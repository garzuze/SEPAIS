...
if(isset($_SESSION['email'])) {
    if ($_SERVER["REQUEST_METHOD"] === "POST") {
        ...
        $query1 = $sql->prepare("UPDATE `sepaisdb`.`sepae_libera_aluno` SET `horario_saida` = now() 
        WHERE (`aluno_id` = ?) and `data` LIKE ?;");
        $query2 = $sql->prepare("UPDATE `sepaisdb`.`responsavel_libera_aluno` SET `horario_saida` = now() 
        WHERE (`aluno_id` = ?) and `data` LIKE ? ;");
        // }
